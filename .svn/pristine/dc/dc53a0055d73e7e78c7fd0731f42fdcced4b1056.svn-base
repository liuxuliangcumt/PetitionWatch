...
import com.realpower.timchat.event.MessageEvent;
...
public class PushUtil implements Observer {
    ...
    private void PushNotify(TIMMessage msg){
        ...
        /*NotificationManager mNotificationManager = (NotificationManager) MyApplication.getmContext().getSystemService(MyApplication.getmContext().NOTIFICATION_SERVICE);
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(MyApplication.getmContext());
        Intent notificationIntent = new Intent(MyApplication.getmContext(), HomeActivity.class);
        notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
                | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent intent = PendingIntent.getActivity(MyApplication.getmContext(), 0,
...
//                .setNumber(++pushNum) //设置通知集合的数量
    ...
    public void reset(){
        NotificationManager notificationManager = (NotificationManager)MyApplication.getmContext().getSystemService(MyApplication.getmContext().NOTIFICATION_SERVICE);
